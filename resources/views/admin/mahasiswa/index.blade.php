<x-app-layout>
    <x-slot name="title">Manajemen Mahasiswa</x-slot>
    <x-slot name="pageTitle">Manajemen Mahasiswa</x-slot>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 mb-6">
        <div class="bg-white rounded-2xl p-4 md:p-5 border border-slate-200 card-hover">
            ...
            <p class="text-sm text-slate-400 mt-0.5">Total Mahasiswa</p>
        </div>
        <div class="bg-white rounded-2xl p-4 md:p-5 border border-slate-200 card-hover">
            <div class="flex items-start justify-between mb-3">
            ...
            <p class="text-2xl font-heading font-semibold text-slate-800">{{ $total }}</p>
            <p class="text-sm text-slate-400 mt-0.5">Mahasiswa Aktif</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-slate-100">
            {{-- Search --}}
            <form method="GET" action="{{ route('admin.mahasiswa.index') }}" class="flex-1 max-w-sm">
                <div class="relative">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-4 py-3.5">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-xl bg-sage-100 flex items-center justify-center flex-shrink-0 text-sage-600 font-semibold text-sm">
                                    {{ strtoupper(substr($m->name, 0, 1)) }}
                                </div>
                                <div class="min-w-0">
                                    <p class="font-medium text-slate-800 truncate">{{ $m->name }}</p>
                                    <p class="text-xs text-slate-400 md:hidden truncate">{{ $m->nip_nim ?? '-' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-3.5 text-slate-600 hidden md:table-cell">
                            <span class="font-mono text-xs bg-slate-100 px-2 py-1 rounded-lg">{{ $m->nip_nim ?? '&mdash;' }}</span>
                        </td>
                        <td class="px-4 py-3.5 text-slate-500 text-xs hidden lg:table-cell">{{ $m->email }}</td>
                        ...
                        <td class="px-4 py-3.5">
                            <div class="flex items-center justify-center gap-1.5">
                                {{-- Edit --}}
                                <button onclick="openEdit({{ $m->id }}, '{{ addslashes($m->name) }}', '{{ $m->email }}', '{{ $m->nip_nim }}')"
                                    title="Edit"
                                ...
                                {{-- Reset Password --}}
                                <form action="{{ route('admin.mahasiswa.reset-password', $m) }}" method="POST"
                                    onsubmit="return confirm('Reset password {{ addslashes($m->name) }} ke \'password\'?')">
                                    @csrf @method('PATCH')
                                ...
                                <form action="{{ route('admin.mahasiswa.destroy', $m) }}" method="POST"
                                    onsubmit="return confirm('Hapus mahasiswa {{ addslashes($m->name) }}?')">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="px-5 py-16 text-center">
                            ...
                            <p class="text-slate-400 text-sm">
                                ...
                                @else
                                    Belum ada data mahasiswa. Klik "Tambah Mahasiswa" untuk memulai.
                                @endif
                            </p>
                            @if(request('search'))
                            <a href="{{ route('admin.mahasiswa.index') }}" class="text-ink-500 text-sm mt-2 inline-block hover:underline">Hapus filter</a>
                            @endif
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="px-5 py-4 border-t border-slate-100 flex items-center justify-between text-sm text-slate-500">
            <span>Menampilkan {{ $mahasiswa->firstItem() }}"“{{ $mahasiswa->lastItem() }} dari {{ $mahasiswa->total() }} mahasiswa</span>
            <div class="flex items-center gap-1">
                ...
                @else
                <a href="{{ $mahasiswa->previousPageUrl() }}" class="w-8 h-8 rounded-lg bg-slate-100 hover:bg-ink-50 hover:text-ink-500 text-slate-500 flex items-center justify-center transition-colors">
                    <i class="fa-solid fa-chevron-left text-xs"></i>
                </a>
                @endif
                ...
                <a href="{{ $url }}" class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-medium transition-colors
                    {{ $page == $mahasiswa->currentPage() ? 'bg-ink-500 text-white' : 'bg-slate-100 hover:bg-ink-50 hover:text-ink-500 text-slate-600' }}">
                    {{ $page }}
                </a>
                @endforeach

                @if($mahasiswa->hasMorePages())
                <a href="{{ $mahasiswa->nextPageUrl() }}" class="w-8 h-8 rounded-lg bg-slate-100 hover:bg-ink-50 hover:text-ink-500 text-slate-500 flex items-center justify-center transition-colors">
    ...
    <div id="modal-tambah" class="hidden fixed inset-0 z-50 items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                <h3 class="font-heading font-semibold text-slate-800">Tambah Mahasiswa Baru</h3>
                <button onclick="closeModal('modal-tambah')" class="text-slate-400 hover:text-slate-600">
                    <i class="fa-solid fa-xmark text-lg"></i>
                </button>
            </div>
            <form action="{{ route('admin.mahasiswa.store') }}" method="POST" class="px-6 py-5 space-y-4">
                @csrf
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Nama Lengkap <span class="text-red-400">*</span></label>
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="contoh: Andi Pratama"
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">NIM <span class="text-red-400">*</span></label>
                    <input type="text" name="nip_nim" value="{{ old('nip_nim') }}" required placeholder="contoh: 2024110001"
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required placeholder="nim@mhs.lmszila.ac.id"
    ...
    <div id="modal-edit" class="hidden fixed inset-0 z-50 items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                <h3 class="font-heading font-semibold text-slate-800">Edit Data Mahasiswa</h3>
            ...
            <form id="form-edit" method="POST" class="px-6 py-5 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">NIM <span class="text-red-400">*</span></label>
                    <input type="text" name="nip_nim" id="edit-nim" required
                ...
                <p class="text-xs text-slate-400 bg-slate-50 rounded-xl px-3 py-2.5">
                    ...
                    Untuk mengubah password, gunakan tombol <span class="font-medium text-amber-600">Reset Password</span> pada baris mahasiswa.
                </p>
                <div class="flex gap-3 pt-1">
    ...
    <script>
        ...
        function openEdit(id, name, email, nim) {
            document.getElementById('form-edit').action = '/admin/mahasiswa/' + id;
        ...
        function previewImport(input) {
            if (input.files && input.files[0]) {
                ...
                if (confirm('Import data mahasiswa dari file "' + fname + '"?\n\nFitur ini memerlukan konfigurasi tambahan.')) {
