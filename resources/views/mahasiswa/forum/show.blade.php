<x-app-layout>
    ...
    <div class="flex items-center gap-3 mb-4 -mt-1">
        <a href="{{ route('mahasiswa.forum.index', $kelas) }}"
        ...
        <span class="text-xs text-slate-400 flex-shrink-0">{{ $topik->komentar->count() }} balasan</span>
    </div>
    ...
    <div class="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden flex flex-col" style="height: calc(100vh - 220px); min-height: 500px;">
        ...
        <div id="chat-messages" class="flex-1 overflow-y-auto px-4 py-4 space-y-3">
            ...
            <div class="flex items-center justify-center h-full">
                <div class="text-center">
                    ...
                    <p class="text-sm text-slate-400">Belum ada balasan. Jadilah yang pertama!</p>
                </div>
            </div>
            @else
            ...
            <div class="flex items-end gap-2 {{ $isMe ? 'flex-row-reverse' : '' }} group">
                ...
                <div class="max-w-[72%] flex flex-col {{ $isMe ? 'items-end' : 'items-start' }}">
                    {{-- Name (hanya untuk pesan orang lain) --}}
                    @if(!$isMe)
                    ...
                    {{-- Bubble --}}
                    <div class="relative {{ $isMe ? 'bg-ink-500 text-white rounded-2xl rounded-br-sm' : ($k->penulis->isDosen() ? 'bg-ink-50 border border-ink-100 text-slate-800 rounded-2xl rounded-bl-sm' : 'bg-white border border-slate-200 text-slate-800 rounded-2xl rounded-bl-sm') }} px-4 py-2.5 shadow-sm">
                        <p class="text-sm leading-relaxed whitespace-pre-line">{{ $k->isi }}</p>
                        {{-- Delete (hanya milik sendiri) --}}
                        @if($isMe)
                        <form method="POST" action="{{ route('mahasiswa.forum.komentar.destroy', [$kelas, $topik, $k]) }}"
                            onsubmit="return confirm('Hapus pesan ini?')"
                            class="absolute -top-2.5 left-0 opacity-0 group-hover:opacity-100 transition-opacity">
                            @csrf @method('DELETE')
                            <button type="submit"
        ...
        <div class="bg-white border-t border-slate-200 px-4 py-3">
            <form action="{{ route('mahasiswa.forum.komentar.store', [$kelas, $topik]) }}" method="POST"
                class="flex items-end gap-2">
                @csrf
                <div class="w-8 h-8 rounded-full bg-sage-500 flex items-center justify-center text-white text-xs font-semibold flex-shrink-0 mb-0.5">
        ...
        <div class="bg-white border-t border-slate-200 px-4 py-3 flex items-center gap-2 text-slate-400">
            ...
            <p class="text-sm">Topik ini dikunci oleh dosen.</p>
        </div>
        @endif
